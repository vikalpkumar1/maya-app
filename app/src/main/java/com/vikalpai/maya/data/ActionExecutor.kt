...
class ActionExecutor(private val context: Context, private val voiceOutput: VoiceOutputManager) {
    ...
    fun run(actionJson: String): String? {
        return try {
            ...
            when (json.getString("type")) {
                ...
                "play_youtube" -> {
                    playYoutubeSearch(json.getString("query"))
    ...
    private fun playYoutubeSearch(query: String) {
        // Opens YouTube's own search results for the query — the YouTube app
        // (if installed) handles this link directly. Actually auto-playing
        // the first result would need YouTube's paid Data API with a quota,
        // so this gets the user one tap away instead: their video is right
